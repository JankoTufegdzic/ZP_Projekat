...
def sendMessageFrame(publicRing, privateRing, email, password):
    ...
    def saveMessage():
        ...
        if file_path:
            ...
            if auth_var.get() == 1:
                flag = sendMessage(email, authPass, text.get("1.0", 'end-1c'), file_path,
                                   publicKeyAuthID=publicKeyAuthId, publicKeyEncrID=publicKeyEncrId, encrAlg=encrAlg,
                                   zip=zip, base64encode=b64)
            else:
                flag = sendMessage(email, "", text.get("1.0", 'end-1c'), file_path, publicKeyAuthID=publicKeyAuthId,
                                   publicKeyEncrID=publicKeyEncrId, encrAlg=encrAlg, zip=zip, base64encode=b64)
    ...
    # Encryption
    encr_var = tk.IntVar()
    encrCheckbox = tk.Checkbutton(sendMessageFrame, text="Select", variable=encr_var,
                                  command=toggle_encryption_visibility)
